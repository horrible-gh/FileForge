...
Run the one-time setup. **Setup also generates the root run launchers**
(`run-server` + `run-client`), so afterwards you can start each stack straight
from the repository root — no need to open `scripts/` to figure out what to run:
...
```powershell
# Windows
.\setup.ps1           # sets up all components AND generates .\run-server.ps1, .\run-client.ps1
.\run-server.ps1      # opens the backend services in separate windows
.\run-client.ps1
```

The launchers are **generated by setup, not committed** — they are regenerated
on every setup run so they always match how setup is configured. To refresh just
the launchers without re-running the full setup, use `./setup.sh
--launchers-only` (Linux/macOS: `FILEFORGE_LAUNCHERS_ONLY=1 ./setup.sh`) or
`.\setup.ps1 -LaunchersOnly` (Windows).

Server and client launchers are intentionally separate. See
[`scripts/README.md`](scripts/README.md) for component-level runners, options,
overrides, build helpers, and stop helpers. The manual steps below remain
available.
