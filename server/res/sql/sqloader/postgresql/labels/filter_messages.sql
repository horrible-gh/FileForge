SELECT m.*, a.email as account_email,
       string_agg(DISTINCT l.label_name, ',' ORDER BY l.label_name) as labels,
       string_agg(DISTINCT l.label_color, ',' ORDER BY l.label_name) as label_colors
...
GROUP BY m.message_uuid, a.email
